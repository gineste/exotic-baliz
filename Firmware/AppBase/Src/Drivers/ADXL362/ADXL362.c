...
#define ADXL362_POWER_CTL_LOW_NOISE(x)       (((x) & ADXL362_CTL_MASK) << 4u)
...
/**@brief Function to Set Noise Control of ADXL362.
 *
 * @param[in] p_eNoiseCtrl - Noise control option.
 *
 * @return Error Code.
 */
e_ADXL362_ErrorCode_t eADXL362_NoiseSet(e_ADXL362_NoiseCtrl_t p_eNoiseCtrl)
{
   ...
   l_u8OldReg = l_au8Buffer[0u];
   l_u8NewReg = l_u8OldReg & (~ADXL362_POWER_CTL_LOW_NOISE(ADXL362_CTL_MASK));
   l_u8NewReg |= ADXL362_POWER_CTL_LOW_NOISE(p_eNoiseCtrl);
   l_au8Buffer[0u] = l_u8NewReg;
   ...
   /* Update ADXL362 Context */
   g_sADXL362.eNoiseCtrl = p_eNoiseCtrl;
...
e_ADXL362_ErrorCode_t eADXL362_NoiseControlSet(e_ADXL362_NoiseCtrl_t p_eNoise)
{
   ...
   l_u8OldReg = l_au8Buffer[0u];
   l_u8NewReg = l_u8OldReg & (~ADXL362_POWER_CTL_LOW_NOISE(ADXL362_CTL_MASK));
   l_u8NewReg |= ADXL362_POWER_CTL_LOW_NOISE(p_eNoise);
   l_au8Buffer[0u] = l_u8NewReg;
   ...
   /* Update ADXL362 Context */
   g_sADXL362.eNoiseCtrl = p_eNoise;
